...
export 'package:native_opencv_kit/native_opencv.dart';

/// A ready-to-use camera screen that:
/// - Streams live camera frames
/// - Runs real-time **blur detection** via [NativeOpencv]
/// - On manual capture, runs **ID-card perspective crop** via [NativeOpencv]
/// - Optionally posts the cropped PNG to an **OCR endpoint** and displays results
///
/// ### Basic usage (no OCR)
/// ```dart
...
///       ocrBaseUrl: 'https://your-ocr-server.com',
///     ),
///   ),
/// );
/// ```
///
/// The screen pops with the cropped RGBA [Uint8List] when the user taps
/// **"Use This"**, or `null` if they dismiss without capturing.
class CameraCaptureScreen extends StatefulWidget {
  ...
  /// OCR endpoint path appended to [ocrBaseUrl].
  /// Defaults to `'/api/ocr/id-card/'`.
  final String ocrPath;

  /// OCR language parameter sent to the server.
  /// Defaults to `'eng+khm'`.
...
class _CameraCaptureScreenState extends State<CameraCaptureScreen>
  ...
  Future<void> _processFrame(CameraImage image) async {
    ...
    // Cache the latest processed frame for manual capture.
    _pendingRgba = rgba;
    _pendingWidth = width;
  ...
  Future<void> _runOcr(Uint8List croppedRgba) async {
    if (widget.ocrBaseUrl == null) return;
    ...
    try {
      ...
      final uri = Uri.parse('${widget.ocrBaseUrl}${widget.ocrPath}');
      final request =
          http.MultipartRequest('POST', uri)
            ..fields['language'] = widget.ocrLanguage
            ..files.add(
  ...
  Uint8List? _convertCameraImageToRgba(CameraImage image) {
    try {
      ...
      debugPrint('[native_opencv_kit] Pixel conversion failed: $e');
      return null;
    }
  }
        ...
        Padding(
          ...
          child: Row(
            ...
            children: [
              ...
              ElevatedButton(
                onPressed: () =>
                    Navigator.of(context).pop(_croppedResultRgba),
